...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}'>
      ...
      <el-form-item :style='{"alignItems":"center","margin":"0 10px","display":"flex"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>赠品商</div>
        <el-select v-model="formSearch.gongyingshang" placeholder="请选择赠品商" :clearable="true">
          <el-option v-for="(item, index) in gongyingshangOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"alignItems":"center","margin":"0 10px","display":"flex"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>入库日期</div>
        <el-date-picker 
          v-model="timeRange"
          type="daterange"
		  :style='{"border":"1px solid #eee","padding":"3px 10px","outline":"none","borderRadius":"0","background":"#fff","width":"auto","justifyContent":"center"}'
          range-separator="至"
          start-placeholder="入库日期起始"
	  ...
	  <el-button v-if="isAuth('zengpinruku','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 35px","margin":"0 10px 0 0","color":"#fff","minWidth":"90px","outline":"none","borderRadius":"0","background":"#20ad9e","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/zengpinrukuAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
...
<script>
  export default {
    ...
    created() {
      ...
      this.$http.get('option/gongyingshang/mingcheng').then(res => {
        if (res.data.code == 0) {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.timeRange.length > 0) {
          searchWhere.rukuriqistart = this.timeRange[0];
          searchWhere.rukuriqiend = this.timeRange[1];
        }
        this.$http.get('zengpinruku/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/zengpinrukuDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
